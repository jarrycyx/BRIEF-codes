...
def save_img(path,img):
    ...
    if postfix in ['.tif','.tiff']:
        ...
        save_video(25,path,img) # 默认为fps=25
    elif postfix in ['.png','.jpg']:
...
def yuv2bgr(filename,height,width,numfrm,startfrm,type):  
    ...
    for i in range(numfrm):
        ...
        yuv_img = yuv_img.reshape((height*3//2, width)).astype('uint8') # YUV 的存储格式为：NV12（YYYY UV）
        bgr_img = cv2.cvtColor(yuv_img, cv2.COLOR_YUV2BGR_I420) # 注意 YUV 的存储格式
