...
from torch.cuda.amp import GradScaler,autocast #TODO 半精度提速
from utils.Logger import MyLogger
from tqdm import tqdm
from utils.Profiler import profile_me
...
class _NFLR_Coding(_NFLR):
    def loss_RateDistortion_UN(self,coords:torch.Tensor,data_gt:torch.Tensor,y:torch.Tensor,Lambda:float) -> torch.Tensor:
        # HACK不需要同时取很多随机数算平均loss，参考数据压缩SOTA源码
...
def eval_performance(data_path_list_list:List[List[str]],compressframework:_BaseCompressFramerwork,save_dir:str,keep_data:bool=True,max=None) -> pd.DataFrame:
    ...
    return performance
def eval_performance_Luoqingming(data_path_list_list:List[List[str]],compressframework:_BaseCompressFramerwork,save_dir:str,keep_data:bool=True) -> pd.DataFrame:
    """Evaluate the given compressframework's performace.
    Args:
        data_path_list_list (List[List[str]]): the data path list to evaluate performance
        save_dir (str): the dir to save compressed data,decompressed data,performance.
    ...
    """
    os.makedirs(save_dir,exist_ok=True)
    metrics = ['data_path','compressed_data_Mbytes','orig_data_Mbytes','compress_ratio',]
    for idx,data_path_list in enumerate(tqdm(data_path_list_list,'Evaluating performance',leave=False,file=sys.stdout)):
        compressed_data_dict = compressframework.compress(data_path_list=data_path_list)
        for steps in compressed_data_dict:
            performance = pd.DataFrame(np.zeros((len(data_path_list_list),len(metrics))),columns=metrics)
            save_dir_current_steps = opj(save_dir,'steps{}'.format(steps))
            os.makedirs(save_dir_current_steps,exist_ok=True)
            compressed_data = compressed_data_dict[steps]
            compressed_data_save_path = opj(save_dir_current_steps,opb(ops(data_path_list[0])[0])+'_compressed')
            # BUG 有时候100倍压缩的存下来却只有50倍
            torch.save(compressed_data,compressed_data_save_path)
            decompressed_data_save_path_list = [opj(save_dir_current_steps,opb(ops(data_path)[0])+'_decompressed'+ops(data_path)[-1]) for data_path in data_path_list]
            decompressed_data = compressframework.decompress(compressed_data=compressed_data,save_path_list=decompressed_data_save_path_list)
            orig_data = read_data_batch(data_path_list)
            orig_data_Mbytes = reduce(lambda x,y:x*y,[os.path.getsize(data_path)/2**20 for data_path in data_path_list])
            compressed_data_Mbytes = os.path.getsize(compressed_data_save_path)/2**20
            compress_ratio = orig_data_Mbytes/compressed_data_Mbytes
            performance.iloc[idx]=[str(data_path_list),compressed_data_Mbytes,orig_data_Mbytes,compress_ratio]
            if not keep_data:
                os.remove(compressed_data_save_path)
                for decompressed_data_save_path in decompressed_data_save_path_list:
                    os.remove(decompressed_data_save_path)
            # mip
            orig_mip_d,orig_mip_h,orig_mip_w = mip(orig_data)
            decompressed_mip_d,decompressed_mip_h,decompressed_mip_w = mip(decompressed_data)
            save_data_batch(orig_mip_d,[opj(save_dir_current_steps,opb(ops(data_path)[0])+'_mip_d'+ops(data_path)[-1]) for data_path in data_path_list])
            save_data_batch(orig_mip_h,[opj(save_dir_current_steps,opb(ops(data_path)[0])+'_mip_h'+ops(data_path)[-1]) for data_path in data_path_list])
            save_data_batch(orig_mip_w,[opj(save_dir_current_steps,opb(ops(data_path)[0])+'_mip_w'+ops(data_path)[-1]) for data_path in data_path_list])
            save_data_batch(decompressed_mip_d,[opj(save_dir_current_steps,opb(ops(data_path)[0])+'_decompressed_mip_d'+ops(data_path)[-1]) for data_path in data_path_list])
            save_data_batch(decompressed_mip_h,[opj(save_dir_current_steps,opb(ops(data_path)[0])+'_decompressed_mip_h'+ops(data_path)[-1]) for data_path in data_path_list])
            save_data_batch(decompressed_mip_w,[opj(save_dir_current_steps,opb(ops(data_path)[0])+'_decompressed_mip_w'+ops(data_path)[-1]) for data_path in data_path_list])
            performance.to_csv(opj(save_dir_current_steps,'performance.csv'))
    return performance
def eval_performance_Shiyigong(data_path_list_list:List[List[str]],compressframework:_BaseCompressFramerwork,save_dir:str,keep_data:bool=True) -> pd.DataFrame:
    """Evaluate the given compressframework's performace.
    Args:
        data_path_list_list (List[List[str]]): the data path list to evaluate performance
        save_dir (str): the dir to save compressed data,decompressed data,performance.
