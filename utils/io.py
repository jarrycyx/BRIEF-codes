...
def read_jp2_seq(path, show_bar=False):
    jp2_seq = glob.glob(path + "*.jp2")
    img3d = []
    if show_bar:
        for jp2_fp in tqdm.tqdm(jp2_seq):
            img = cv2.imread(jp2_fp, cv2.IMREAD_GRAYSCALE)
            img3d.append(img)
    else:
        for jp2_fp in jp2_seq:
            img = cv2.imread(jp2_fp, cv2.IMREAD_GRAYSCALE)
            img3d.append(img)
...
def read_data_batch(data_path_list: List[str]) -> np.ndarray:
    """根据后缀决定读取方法，读取数据，并将其shape转换为标准形式
...
def save_data_batch(data_batch: np.ndarray,save_path_list: List[str]):
    """传入标准形式(batch_size,data_channel,*coords_shape)的数据，
    并根据后缀决定保存方法，保存数据
    """
...
def normalize_data(data:np.ndarray,name: str,min=None,max=None) -> Tuple[torch.Tensor,dict]:
    """传入刚刚读入的数据（未归一化），根据name选择归一化方法，对数据归一化
    """
    #TODO 更多的归一化方法
...
def invnormalize_data(data:torch.Tensor,sideinfos:dict,name: str) -> np.ndarray:
    """传入神经表示计算出的数据，根据name选择逆归一化方法，对数据逆归一化
    """
...
if __name__=="__main__":
    a = read_data_batch(["/ssd/2/cyx/Dataset/xray-HiPCT/52.76um_FO-20.129_lung-left_upper-lobe_pag-0.15_0.03_jp2_/"])
    print(a.shape)