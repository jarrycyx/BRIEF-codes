...
def create_flattened_coords(coords_shape:Tuple,mode='n11') -> torch.Tensor:
    """产生拉直形式(prod(*coords_shape),len(coords_shape))的坐标
    """
    if mode == 'n11':
        minimum = -1
        maximum = 1
...
def crop_data(data:torch.Tensor,ps_h:int,ps_w:int,ol_h:int,ol_w:int,ps_d:int=None,ol_d:int=None) -> Tuple[torch.Tensor,tuple]:
    """传入标准形式(batch_size,data_channel,*coords_shape)的数据，根据ol_参数与ps_参数对数据进行带有 overlap 的分割，
    并将它们合并为(batch_size,pc_d,pc_h,pc_w,data_channel,ps_d,ps_h,ps_w)的数据
    """
