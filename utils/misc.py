...
from utils.find_weight import find_weight
...
def parse_weight(data:np.ndarray,weight_type_list:List[str]):
    ...
    for weight_type in weight_type_list:
        if 'quantile' in weight_type:
            ...
            l,h = range_limit(data,[l,h])
            weight[(data>=l) * (data<=h)] = scale
        elif 'Find' in weight_type:
            method,thr,gap_z,gap_xy = weight_type.split('_')
            thr,gap_z,gap_xy = int(thr),int(gap_z),int(gap_xy)
            weight = find_weight(data,thr,gap_z,gap_xy,method)
            # # 亮度进行限制
            # mid_x,mid_value = 4000,0.1
            # a = -np.log(mid_value)/mid_x
            # weight = weight*np.exp(-a*data)
...
def alloc_param(data_chunk_list:List[dict],param_size:float,param_alloc:str,param_size_thres:float):
    if param_alloc == 'equal':
    ...
    elif "aoi_thres" in param_alloc:
        thres = int(param_alloc.split("_")[-1])
        aoi_list = []
        valid_block_num = 0
        for data_chunk in data_chunk_list:
            aoi_list.append((data_chunk['data']>0).sum())  # 已经preprocess 所以用0做阈值
            valid_block_num += (aoi_list[-1]>thres)
            
        for aoi, data_chunk in zip(aoi_list, data_chunk_list):
            data_chunk['param_size'] = param_size * 1 / valid_block_num if aoi > thres else 0
        ...
        for data_chunk in data_chunk_list:
            aoi_list.append((data_chunk['data']>0).sum())  # 已经preprocess 所以用0做阈值
        aoi_sum = np.sum(aoi_list)
        for aoi,data_chunk in zip(aoi_list, data_chunk_list):
            data_chunk['param_size'] = param_size*aoi / aoi_sum
            data_chunk['param_size'] = float(data_chunk['param_size'])
...
def cal_aoi(data_gt:np.ndarray,data_hat:np.ndarray,thres:float=1):
    ...
    return aoi
def cal_psnr(data_gt:np.ndarray,data_hat:np.ndarray,data_range,psnr_type:list):
    data_hat = np.copy(data_hat)
    data_gt = np.copy(data_gt)
    if psnr_type != []:
        data_gt = data_gt.clip(psnr_type[0],psnr_type[1])
        data_hat = data_hat.clip(psnr_type[0],psnr_type[1])
...
def cal_iou_acc_pre(data_gt:np.ndarray,data_hat:np.ndarray,thres:float=1):
    ...
    pre = 1.0*tp/(tp+fp)
    return iou, acc, pre
def cal_mse_mape(data1:np.ndarray,data2:np.ndarray,thres:float):
    lower1 = data1[data1<thres]
    lower2 = data2[data1<thres]
    msel = ((lower1-lower2)**2).mean()
    mapel = (np.abs(lower1[lower1!=0]-lower2[lower1!=0])/lower1[lower1!=0]).mean() + (np.abs(lower2[lower2!=0]-lower1[lower2!=0])/lower2[lower2!=0]).mean()
    mapel = mapel/2
    higher1 = data1[data1>=thres]
    higher2 = data2[data1>=thres]
    mseh = ((higher1-higher2)**2).mean()
    mapeh = (np.abs(higher1-higher2)/higher1).mean()
    return msel,mseh,mapel,mapeh
def cal_mse(data1:np.ndarray,data2:np.ndarray):
    mse = ((data1-data2)**2).mean()
    return mse

def eval_performance(steps:int, orig_data:np.ndarray, decompressed_data:np.ndarray, Log,
    aoi_thres_list:List[int], mse_mape_thres_list:List[int], psnr_thres_list:List[List], iou_acc_pre_thres_list:List[int], ssim:bool):
    # performance
    performance_dict = {}
    performance_dict['steps'] = steps
    max = get_type_max(orig_data)
    ...
    for aoi_thres in aoi_thres_list:
        ...
        performance_dict['aoi-{}'.format(aoi_thres)] = aoi_value
    aoi_value = cal_aoi(orig_data,decompressed_data,1)
    Log.log_metrics({'aoi':aoi_value},steps)
    performance_dict['aoi'] = aoi_value
    # psnr
    for psnr_type in psnr_thres_list:
        if psnr_type != []:
            psnr_value = cal_psnr(orig_data,decompressed_data,max,psnr_type)
            Log.log_metrics({'psnr-{}'.format(str(psnr_type)):psnr_value},steps)
            performance_dict['psnr-{}'.format(str(psnr_type))] = psnr_value
    psnr_value = cal_psnr(orig_data,decompressed_data,max,[])
    Log.log_metrics({'psnr':psnr_value},steps)
    performance_dict['psnr'] = psnr_value
    # mse mape
    for mse_mape_thres in mse_mape_thres_list:
        msel,mseh,mapel,mapeh = cal_mse_mape(orig_data,decompressed_data,mse_mape_thres)
        Log.log_metrics({'msel-{}'.format(str(mse_mape_thres)):msel},steps)
        Log.log_metrics({'mseh-{}'.format(str(mse_mape_thres)):mseh},steps)
        Log.log_metrics({'mapel-{}'.format(str(mse_mape_thres)):mapel},steps)
        Log.log_metrics({'mapeh-{}'.format(str(mse_mape_thres)):mapeh},steps)
        performance_dict['msel-{}'.format(str(mse_mape_thres))] = msel
        performance_dict['mseh-{}'.format(str(mse_mape_thres))] = mseh
        performance_dict['mapel-{}'.format(str(mse_mape_thres))] = mapel
        performance_dict['mapeh-{}'.format(str(mse_mape_thres))] = mapeh
    mse_value = cal_mse(orig_data,decompressed_data)
    Log.log_metrics({'mse':mse_value},steps)
    performance_dict['mse'] = mse_value
    # iou
    for thres in iou_acc_pre_thres_list:
        iou_value, acc_value, pre_value = cal_iou_acc_pre(orig_data,decompressed_data,thres)
        Log.log_metrics({'iou-{}'.format(str(thres)):iou_value},steps)
        performance_dict['iou-{}'.format(str(thres))] = iou_value
        Log.log_metrics({'accuracy-{}'.format(str(thres)):acc_value},steps)
        performance_dict['accuracy-{}'.format(str(thres))] = acc_value
        Log.log_metrics({'precision-{}'.format(str(thres)):pre_value},steps)
        performance_dict['precision-{}'.format(str(thres))] = pre_value
    iou_value, acc_value, pre_value = cal_iou_acc_pre(orig_data,decompressed_data,1)
    Log.log_metrics({'iou':iou_value},steps)
    performance_dict['iou'] = iou_value
    Log.log_metrics({'accuracy':acc_value},steps)
    performance_dict['accuracy'] = acc_value
    Log.log_metrics({'precision':pre_value},steps)
    performance_dict['precision'] = pre_value
    # ssim
    if ssim:
        ...
        if len(orig_data.shape) == 3:
            ...
            ssim_value = ssim_calc(data1,data2,max)
