...
class FFN(nn.Module):
    ...
    """
    def __init__(self,coords_channel=3,data_channel=1,embsize=256,scale=10,features=256,layers=5,skip=False):
        super().__init__()
        self.skip = skip
        self.skip_layer = (layers-1)//2 if skip else -1
...
class SIRENFT(nn.Module):
    ...
    def check_param_count(param_count,coords_channel,data_channel,layers,res,ratio,**kwargs):
        # pc = c*(f+p)+f+p+(f+p)*f+f+(l-3)*(f**2+f)+f*d+d,f至少为1才能用该模型
        features = 1
        first_feature = int(features*ratio)
...
class SIREN_Pyramid(nn.Module):
    ...
    def calc_param_count(coords_channel,data_channel,features,layers,res,features_dis,**kwargs):
        if res: # 我没改
            ...
            # pc = c*f+f+[f*(f-d)+(f-d)+(f-d)*(f-2*d)+(f-2*d)+...](l-2层)+(f-(l-2)*d)*o+o
            # 其中[f*(f-d)+(f-d)+(f-d)*(f-2*d)+(f-2*d)+...] = |求和|(f+(1-d)/2-i*d)**2|求和|-(l-2)*(1+d)**2/4
            # = (l-2)*f**2+[(1-d)*(l-2)-(l-2)*(l-3)*d]*f+[(l-2)*(1-d)**2/4-(l-2)*(l-3)*d+(l-2)*(l-3)*(2*l-5)*d**2/6-(l-2)*(1+d)**2/4]
            # 最终为：pc = (l-2)*f**2+[c+1+(1-d)*(l-2)-(l-2)*(l-3)*d+o]*f+[(l-2)*(1-d)**2/4-(l-2)*(l-3)*d+(l-2)*(l-3)*(2*l-5)*d**2/6-(l-2)*(1+d)**2/4]-(l-2)*d*o+o
        return int(param_count)
    @staticmethod
    def check_param_count(param_count,coords_channel,data_channel,layers,res,features_dis,**kwargs):
        features = 1 + (layers-2)*features_dis  # 最后一层至少为1
...
class SIRENPS(nn.Module):
    ...
    def calc_features(param_count,coords_channel,data_channel,layers,res,ratio,**kwargs):
        if res: # 没改
            ...
            limit = l1*l2+l2
            for i in range(layers-2):
                l1 = features*ratio**(layers-2-i)     # 因为int而出现压缩率不能达到要求
...
class CropConv3dStridedown(nn.Module):
    """
    模仿他的源代码 J. Ballé, D. Minnen, S. Singh, S.J. Hwang, N. Johnston:"Variational Image Compression with a Scale Hyperprior"
        ...
        for ind in range(Downsample_layer):
            if ind == 0 :
                self.net.append(nn.Sequential(
                    ...
                    nn.ReLU()# GDN(y_channel) #FIXME GDN的to函数太费时间
                ))
    ...
    def forward(self, data,bs:int=0):
        """
        Args:
            bs (int, optional):当gpu不足时，在for循环内小批量计算y，最终合并
        """
...
def init_hz(**kwargs) -> Union[PlainConv3dChannelShrink,]:
...
def init_gy(**kwargs) -> Union[PlainConv3dMeanScale,]:
